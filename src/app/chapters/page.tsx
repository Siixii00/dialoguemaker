...
export default function ChapterSelectionPage() {
  ...
  const handleGoogleLogin = () => {
    if (!adminConfig?.googleClientId) {
      setAuthError("系統尚未設定 Google Client ID");
      return;
    }
    ...
    googleAuthUrl.searchParams.set("client_id", adminConfig.googleClientId);
  ...
  const handleEmailSubmit = async () => {
    ...
    if (loginEmail !== adminConfig?.adminEmail) {
      setAuthError("此 Email 非管理員帳號");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        if (data.error === "2FA 已設定，請使用驗證碼登入") {
  ...
  const handle2FAVerify = async () => {
    ...
    try {
      const res = await fetch("/api/auth/verify-2fa", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: adminConfig?.adminEmail, code: verifyCode }),
      });
        ...
        <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center">
          <div className="p-8 rounded-lg max-w-md w-full mx-4" style={{ background: "rgba(31,37,47,0.95)", border: "1px solid rgba(211,188,142,0.3)" }}>
            ...
            <div className="space-y-4">
              <div>
                <label className="font-display text-sm text-primary">管理員 Email</label>
                <input
                  type="email"
                  value={loginEmail}
              ...
              >
                {loading ? "處理中..." : "使用 Email 登入"}
              </button>

              <div className="relative flex items-center justify-center my-4">
                <div className="border-t border-primary/20 w-full"></div>
                <span className="px-4 text-muted text-sm bg-[rgba(31,37,47,0.95)] relative">或</span>
                <div className="border-t border-primary/20 w-full"></div>
              </div>
